...
def create_student(student_id, name, age, seat: Tuple[int, int]):
    ...
    print(f"Student with id {student_id} ahs beeen succesfully created")
...
def update_students(student_id:str, name: str = None, age: int= None, seat: Tuple[int, int] = None, graduated: bool = None):
    ...
    print(f"student with {student_id} has beeen updated")
    # if age is not None:
    #     s['age'] = age
    # if seat is not None:
    #     s['seat']
...
def delete_student(student_id):
    if student_exists(student_id):
        ...
        print(f"Student with ID:{student_id} has been deleted succesfully")
    else:
        print(f"sudent with Id:{student_id} does not exist")
...
def add_score(student_id, score):
    ...
    if 0 <= score <= 100:
        ...
        print(f"score has been added succesfully and the new average is {avg(students[student_id]['scores'])}")
    else:
...
def remove_score(student_id, index):
    ...
    if 0<= index <= len(s):
        removed = s.pop(index)
        print(f'then valur {removed} has ben deleted succesfulluy')
    else:
...
def add_course(student_id, course):
    ...
    print(f'{course} has been added succesfully')
...
def remove_course(student_id, index):
    ...
    if 0<= index <= len(s):
        removed = s.pop(index)
        print(f'then value {removed} has ben deleted succesfulluy')
    else:
        print("Invalid score index")
...
def main():
    while True:
        ...
        print("6) Add score or Remove core")
        print("0 Exit")
